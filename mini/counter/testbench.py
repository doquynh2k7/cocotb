...
async def basic_count(dut):
    ...
    for cnt in range(50):
        await RisingEdge(dut.clk)
        count_val = dut.counter.value
        print("count = "+str(count_val.integer))
        mod_cnt = cnt % 16
        assert count_val.integer == mod_cnt, "counter result is incorrect: %s != %s" % (str(dut.counter.value), mod_cnt)
